...
<div class="flex h-full">
  <!-- Narrow sidebar -->
  @section('body')
  @include('_partials._admin.sidebar')
  @include('_partials._admin.mobile-menu')

  <!-- Content area -->
  <div class="flex flex-col flex-1 overflow-hidden">
    @include('_partials._admin.page-header')
    <!-- Main content -->
          ...
          <div class="w-full px-4 py-6 mx-auto sm:px-6 md:px-8 md:flex md:items-center md:justify-between">
            <div class="flex-1 min-w-0">
              <h2 class="text-2xl font-semibold text-gray-900">
                <a class="text-indigo-700 hover:text-indigo-300" href="/admin-order-index">Rendelések</a>
                <span class="font-medium text-indigo-300">/</span>
                Rendelés összesítő
              </h2>
            </div>
          </div>
          {{-- @include('_partials._admin.page-heading-details') --}}
          @include('_partials._admin.order-summary')
        </section>
      </main>

      <!-- Secondary column (hidden on smaller screens) -->
      @include('_partials._admin.order-summary-sidebar')
    </div>
  </div>
</div>
@endsection
